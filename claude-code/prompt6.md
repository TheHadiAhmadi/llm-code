...
2. Security Check:
   ...
   - Verify that the command is not one of the banned commands: ${oN1.join(", ")}.
...
- If the output exceeds ${Fe} characters, output will be truncated before being returned to you.
  - VERY IMPORTANT: You MUST avoid using search commands like \`find\` and \`grep\`. Instead use ${je}, ${be}, or ${ju} to search. You MUST avoid read tools like \`cat\`, \`head\`, \`tail\`, and \`ls\`, and use ${BI.name} and ${q7.name} to read files.
